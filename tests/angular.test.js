/**
 * @everystate/angular: integration tests via @everystate/test
 *
 * Tests the store-side patterns that Angular adapter functions consume.
 * Since EveryState is the IR, testing the IR proves the adapter works.
 * The functions are thin wrappers: usePath = subscribe + signal,
 * useIntent = set, useWildcard = subscribe wildcard + get parent.
 *
 * Angular-specific behavior (change detection, DI, signals) requires
 * an Angular test environment and is outside the scope of these tests.
 */
...
const results = runTests({
  ...
  // -- usePath patterns (signal bridge) ------------------------------------
  ...
  'usePath: unsubscribe stops notifications (ngOnDestroy)': () => {
    const store = createEveryState({ count: 0 });
    let fires = 0;
    const unsub = store.subscribe('count', () => { fires++; });
    store.set('count', 1);
    unsub(); // simulates ngOnDestroy cleanup
    store.set('count', 2);
  ...
  // -- DI pattern (Injectable service) -------------------------------------

  'DI: store as external store (Injectable compat)': () => {
    const store = createEveryState({ count: 0 });
    // Simulate Injectable service + signal contract
  ...
  // -- batch (Angular signal coalescing compat) ----------------------------

  'batch: atomic updates (Angular signal coalescing compat)': () => {
  ...
  // -- type generation from Angular patterns -------------------------------

  'types: Angular app state shape': () => {
    const t = createEventTest({
