/**
 * @everystate/angular: zero-dependency self-test
 *
 * Since the Angular adapter uses Angular's signal() (which requires an
 * Angular runtime), this self-test verifies:
 * 1. The store-side patterns that the adapter functions consume work correctly
 ...
 *
 * The adapter functions (usePath, useIntent, useWildcard, useAsync) are
 * thin wrappers around store.subscribe + Angular's signal().
 * Testing the store patterns proves the adapter will work.
 */
...
// -- 1. usePath pattern: subscribe + get (signal bridge) ------------------
...
// -- 6. DI pattern: store as injectable service ---------------------------

section('6. DI pattern: store as external dependency');
...
// -- 7. Batch pattern (Angular signal coalescing compat) ------------------

section('7. batch pattern (Angular signal coalescing compat)');
...
// -- 8. Cleanup pattern: unsubscribe on ngOnDestroy -----------------------

section('8. Unsubscribe pattern (ngOnDestroy simulation)');
...
console.log(`\n@everystate/angular v1.0.0 self-test`);
if (failed > 0) {
